...
config.register_player(name="unif", algorithm=opp)
...
for batch in range(NUM_BATCHES):
    ...
    for game in range(NUM_GAMES_PER_BATCH):
        config.players_info[1]['algorithm'] = RLPlayer(epsilon=e_0 * d ** batch)
...
print()
plt.title('Win-rate of RL agent against RaisedPlayer')
plt.xlabel('batches')
plt.ylabel('percent of games won')
plt.plot(win_rates)
plt.savefig('RL_winrate_over_time_raised.jpg')
